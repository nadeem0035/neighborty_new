...
class calendar extends calendarExtensions
{
    ...
    public function __construct($db_server, $db_username, $db_password, $db_name, $table, $condition=false, $cf='')
    {
        ...
        $this->table = $table;
        $this->uid = $_SESSION['logged_in']['id'];
        ...
        $end = @$this->filter['end'];
        $uid = @$this->filter['user_id'];


       // echo $_SESSION['logged_in']['id'].'!=' .$start.'==='.$end;

        // discontinued due performance issues, it was used to retrieve 6 months data
       ...
       // $session_data = $this->session->userdata('logged_in');
        ...
        if($this->condition == false)
        {
            ...
            if(strlen($start) !== 0 && strlen($end) !== 0)
            {
                ...
                $dc = sprintf(" WHERE user_id = $uid AND start >= '%s' AND end <= '%s'", addslashes($start), addslashes($end));
            }
            ...
            if(strlen($start) !== 0 && strlen($end) !== 0)
            {
                $dc = sprintf(" AND user_id = $uid AND  start >= '%s' AND end <= '%s'", addslashes($start), addslashes($end));
            }
