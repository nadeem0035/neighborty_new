						...
						foreach($v['fields'] as $if_label => $if_pattern)
						{
							...
							foreach($labels as $l)
							{
								if($l)
								{
									if($dfV)
									{
										if($dfV == $values[$t])
										{
											$form .= '<option selected value="'.$values[$t].'">'.$labels[$t] . '</option>' . $this->newLine;
										} else {
							...
							if(isset($rows))
							{
								foreach($rows as $r)
								{
									if($dfV)
									{
										if($dfV == $r[$fileds_table_name])
										{
											$form .= '<option selected value="'.$r[$fileds_table_name].'">'.$r[$fileds_table_name] . '</option>' . $this->newLine;
										} else {
