...
final class Worldpay {
    ...
    public function __construct($service_key = false, $timeout = false) {
        ...
        $this->_ci->load->config('worldpay');
        $service_key = $this->_ci->config->item('service_key');
        ...
        $disable = $this->_ci->config->item('disableSSLCheck');
        $this->disableSSLCheck($disable);
        //
    }
    ...
    private function sendRequest($action, $json = false, $expectResponse = false, $method = 'POST') {
        ...
        if (isset($response["httpStatusCode"])) {
            ...
            if ($response["httpStatusCode"] != 200) {
                self::onError(
                    false, $response["message"], $info['http_code'], $response['httpStatusCode'], $response['description'], $response['customCode']
                );
            }
    ...
    $error = false, $message = false, $code = null, $httpStatusCode = null, $description = null, $customCode = null
    ) {
        ...
        if (version_compare(phpversion(), '5.3.0', '>=')) {
            ...
            $error_message, $code, null, $httpStatusCode, $description, $customCode
            );
        } else {
            throw new Exception(
            $error_message, $code
...
class WorldpayException extends Exception {
    ...
    $message = null, $code = 0, Exception $previous = null, $httpStatusCode = null, $description = null, $customCode = null
    ) {
