...
class Booking extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('Listings_model', 'listing_m', true);
    ...
    public function calendar_appointments($id)
    {

        echo  $appointment  = $this->listing_m->ActivecalendarAppointments($id);

    }

    function availability_popup_canelnder()
    {
        $data['avail_time_from'] = $this->input->post('time_from');
        $data['avail_time_to'] = $this->input->post('time_to');

       return $this->load->view('front_end/availpopup',$data);
    }

    public function get_appointment_hours($user_id)
    {
        $session_data = $this->session->userdata('logged_in');
        $uid = $session_data['id'];

        $date = $this->input->post('date');
        $appointment_times = $this->listing_m->get_hours($date,$user_id);
        $availability_id = $appointment_times[0]->avail_id;
        $availability_times = $this->listing_m->get_appointment_time($availability_id);
        $pending_apps = $this->listing_m->get_pending_appointments($availability_id, $uid);
        calculate_appointment_time($appointment_times,$availability_times,$pending_apps);
    }
    ...
    public function detail()
    {
        ...
        set_extra_js("$('.send_msg').click(function () { $(this).hide();$('.text_fields').show(); }); ");
        set_extra_js("$('.slide').slickLightbox({ itemSelector : 'a', navigateByKeyboard  : true });");

        $data->users_avatar = $this->config->item('users_avatar');
        $data->listing = $this->listing_m->get_listing_detail($listing_id);

        $type = $data->listing->property_type;;
        $sub = $data->listing->property_sub_type;

        $data->list_amenities = $this->listing_m->amenities_by_category($type,$sub);
      //  pre($data->list_amenities);

        $agent_id = $data->listing->user_id;
        $listing_location = $data->listing->city_town;
        $data->featured_agents_by_location = $this->agents->getFeaturedAgentsByLocation($listing_location);
        $data->related  = $this->listing_m->agent_related_listings($agent_id,$listing_id);
        $data->userdetail = $this->common->get_row_with_specific_data("*", 'users', 'id', (int) $data->listing->user_id);
        $amenities = $this->listing_m->get_list_amenities_name((int) $listing_id);
        $data->categories = $this->listing_m->get_all_cats();
        //echo count($amenities);
        $data->documents = $this->listing_m->get_list_documents((int) $listing_id);
        $data->floorplans = $this->listing_m->get_list_plans((int) $listing_id);
        $data->featured  = $this->listing_m->FeaturedListings(10);
        ...
        if( $this->session->userdata('logged_in') )
        {
            ...
            $bdata['listing_id ']  = $listing_id;
            $this->common->commonSave('user_listing_activity',$bdata);

            $this->listing_m->updateViews($listing_id);

            $data->wishlists = $this->wishlist->getWishlistByListingId($listing_id,$user_id);
            $data->applied  = $this->listing_m->AppliedListings((int) $listing_id,$user_id);

        }

        $data->appointments = $this->listing_m->appointments((int) $agent_id);
      /*echo '<pre>';print_r($data);
        ...
        $data->detail_review = $this->listing_m->get_listing_detail_review((int) $listing_id);
        $data->reviews_all = $this->listing_m->get_list_reviews_all((int) $listing_id);
        //echo $this->db->last_query();
        $review = $this->listing_m->get_listing_review((int) $listing_id);
        ...
        $pics = $this->listing_m->get_list_images((int) $listing_id);
        ...
        $data->home_types = $this->listing_m->home_types_search();
//        //  $data['room_types'] = $this->listing->room_types_search();
     //   $data->amenities  = $this->listing_m->amenities();
        //Title and meta description
        //$this->seo->SetValues('OgImg', $data->listing->preview_image_url);

        $this->seo->SetValues('OgImg', base_url('assets/media/properties/thumbs/'.$data->listing->preview_image_url));
        $this->seo->SetValues('Title', ucfirst($data->listing->listing_name) . ' - Apartments for Rent in ' . ucfirst($data->listing->state_province));
        $this->seo->SetValues('Keywords', "property in " .$data->listing->city_town.", house for ".$data->listing->property_type.", houses in ".$data->listing->typed_address);
        $this->seo->SetValues('Description', ucfirst($data->listing->home_type) . ' for Rs ' . $data->listing->price . ". " . ucfirst(substr($data->listing->summary, 0, 140)));
    ...
    function confirm_booking() {
        ...
        if ($this->session->userdata('logged_in') && $this->input->post('slug')) { //
            ...
            set_extra_js("Stripe.setPublishableKey('sk_test_ZB6rrbObArTsMGGikfe7fAQV');");
            ...
            if ($data->listing->active != 'Publish') {
                ...
                //Title and meta description
                $this->seo->SetValues('Title', 'Vacation rentals, private rooms, sublets by the night - Accommodations on Zoney');
    ...
    function process_request() {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            //Title and meta description
            $this->seo->SetValues('Title', 'Book rental property for your Vacation - Zoney');
            ...
            if ($this->input->post('transaction_through') == 'card') { //Stripe payment method
                ...
                try {
                    ...
                    if ($response['paymentStatus'] === 'SUCCESS' || $response['paymentStatus'] === 'AUTHORIZED') {
                        ...
                        $booking = $this->Booking_model->booking($booking_cart);
                        if($booking_cart['is_featured'] == '1'){
                          $this->Listings_model->update_ufeature_status($guest_id);
    ...
    function success() {
        ...
        if ($this->input->get('token')) {
            ...
            //Title and meta description
            $this->seo->SetValues('Title', 'Book rental property for your Vacation - Neighborty');
    ...
    function reservation_requests($bid = NULL, $message = NULL) {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $this->seo->SetValues('Title', 'Guest Reservation Requests - Neighborty');
            ...
            if ($this->Booking_model->update_reservation_status($bid, $session_data['id'], array('status' => 'approved', 'key_exchange' => $keyExhangeInfo))) {
                ...
                if ($booking->transaction_through == 'card') {
                    ...
                    try {
                        ...
                        SendDefaultMessage($booking->host_id, 'Your First Listing  has been Approved', 'Congratulations! You have been approved to list your first property on the exclusive Neighborty network.
                        ...
                        We are very excited to have you join us and be part of making hospitality reach the Neighborty standard.<br /> ' , 'Congratulation');
