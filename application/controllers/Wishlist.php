...
class Wishlist extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->seo->SetValues('Title', $session_data['full_name']."'s Wish Lists - Neighborty");
    }
    ...
    public function index()
    {
        if( $this->session->userdata('logged_in') ) {
            ...
            $session_data = $this->session->userdata('logged_in');
            $uid = $session_data['id'];
            $name = $session_data['first_name']."'".'s Wishlist';
            ...
            $data['pictures'] = $this->listing_m->get_list_images((int) $id);



          //  echo '<pre>';print_r($data['deal']);

            $data['wishlist_categoris'] = $this->wishlist->getWishlistCategories();
            $data['wishlists'] = $this->wishlist->getWishlistsByUserId();

            if (empty($data['wishlists'])) {

                $user_data = array('created_by' => $uid, 'name' => $name, 'status' => 1);
                $this->wishlist->setDefaultWishlist($user_data);
                $data['wishlist_categoris'] = $this->wishlist->getWishlistCategories();

            }

            //echo count($data['wishlists']);
    ...
    function wishlist()
    {
        ...
        if( $this->session->userdata('logged_in') )
        {
            ...
            //Add Js/Css
            add_css(array('light.css'));
            add_js(array('jquery.slimscroll.min.js', 'jquery.validate.min.js', 'general.js','custom_maps.js','autocomplete_map.js'));
