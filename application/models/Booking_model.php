...
Class Booking_model extends CI_Model {
    ...
    public function add_package_stat($data) {
        $query = $this->db->insert('booking', $data);
        if ($query) {
            return true;
        } else {
    ...
    public function add_pack_stat($data) {
        $query = $this->db->insert('package_stats', $data);
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function update_package_stat($data_array,$guest_id) {
        $this->db->select('*');
        ...
        if ($query->num_rows() == 1) {
            $this->db->where('agent_id', $guest_id);
            if ($this->db->update('package_stats', $data_array)) {
                return true;
            } else {
                return false;
            }
