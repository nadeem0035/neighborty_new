...
Class Wishlists_model extends CI_Model
{
    ...
    public function is_user_has_wishlist($uid,$lid)
    {
        $this->db->select('*');
    ...
    public function getWishlistByListingId($listing_id,$user_id)
    {
        $this->db->select('count(wishlists.id) as total');
        $this->db->from('wishlists');
    ...
    public function getWishlistsByUserId()
    {
        ...
        $uid = $session_data['id'];
        //$data = array(1,$uid);
        $this->db->select('wishlist_categories.id as categoryid,wishlist_categories.name,listing.id as listingid,listing.title,listing.slug,listing.property_type,listing.preview_image_url,listing.bedrooms,listing.bathrooms,listing.area_sqrft,listing.price,wishlists.id,count(wishlists.id) as total');
        $this->db->from('wishlist_categories');
...
//        $this->db->select('wishlists.id,wishlists.note,wishlists.status,listing.id as listingid,listing.title,listing.beds,listing.price,listing.preview_image_url,listing.slug,listing.home_type,
...
/*** This code Block is just Commented For Dummy Data .When Live comment this Query***/
        ...
        // $this->db->select('wishlists.id,wishlists.note,wishlists.status,listing.id as listingid,listing.summary,listing.bedrooms,listing.bathrooms,listing.accommodates,listing.title,listing.beds,listing.price,listing.preview_image_url as image,listing.slug,listing.home_type, listing.room_type,listing.typed_address,listing.latitude,listing.longitude');
        $this->db->select('wishlists.id,wishlists.note,wishlists.status,listing.id as listingid,listing.summary,listing.bedrooms,listing.bathrooms,listing.title,listing.price,listing.preview_image_url as image,listing.slug,listing.property_location,listing.area_sqrft,listing.property_type,listing.latitude,listing.longitude');
        $this->db->from('listing');
        // $this->db->where('wishlists.wishlist_categories',$id);
    ...
    public function renterwishlistDetails($id)
    {

        $this->db->select('wishlists.id,wishlists.note,wishlists.status,listing.id as listingid,listing.bedrooms,listing.bathrooms,listing.title,listing.price,listing.preview_image_url,listing.slug,listing.area_sqrft,listing.property_type');
        $this->db->from('listing');
        $this->db->like('wishlists.user_id', $id);
        $this->db->join('wishlists','listing.id = wishlists.listing_id','left');
        $query = $this->db->get();
    ...
    public function catDetails($id){
        ...
        $this->db->select('wishlist_categories.id as categoryid,wishlist_categories.name,wishlist_categories.visibality,wishlists.id,count(wishlists.id) as total,listing.preview_image_url');
        $this->db->from('wishlist_categories');
