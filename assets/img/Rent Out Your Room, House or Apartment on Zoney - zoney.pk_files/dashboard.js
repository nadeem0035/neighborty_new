        ...
        else
        {
            ...
            $.ajax({
                ...
                success: function (data, textSatus, jqXHR) {
                    ...
                    if (data.msg) {
                        $("#listing_response").html('<div class="text-left" style="color: #3c763d"><button class="close" data-dismiss="alert"></button>Preview image upload successfully</div>');

                    } else {
                        $("#listing_response").html('<div class="alert alert-danger"><button class="close" data-dismiss="alert"></button>'+data.error+'</div>');
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
...
//function submitFile() {
    ...
    var formUrl = site_url + 'listings/listing_images_upload';
    ...
    $.ajax(
        {
            ...
            success: function(data) {
                ...
                if (data.msg == 'success')
                {
                    $("#listing_response").html('<div class="alert alert-success"><button class="close" data-dismiss="alert"></button>Gallery images upload successfully</div>');
...
$(".submit").click(function(){
    var summryValue = $('#txt-content').val();
    if(summryValue == ""){
        $('.simditor').addClass('has-error');
        addvalidate();
    } else {
        $('.simditor').removeClass('has-error');
        addvalidate();
    }
