...
require_once (XCRUD_PATH . '../../config.php'); // configuration variables
...
class Xcrud {
    ...
    public function alert($columns = '', $cc = '', $subject = '', $message = '', $link = false, $field = false, $value = false, $mode = 'all') {
    ...
    public function mass_alert_create($email_table = '', $email_column = '', $emeil_where = '', $subject = '', $message = '', $link = false, $field = false, $value = false) {
        return $this->mass_alert($email_table, $email_column, $emeil_where, $subject, $message, $link, $field, $value, $table, 'create');
    }

    public function mass_alert_edit($email_table = '', $email_column = '', $emeil_where = '', $subject = '', $message = '', $link = false, $field = false, $value = false) {
        return $this->mass_alert($email_table, $email_column, $emeil_where, $subject, $message, $link, $field, $value, $table, 'edit');
    }
    ...
    protected function _insert($postdata, $no_processing = false, $no_processing_fields = array()) {
        ...
        foreach ($postdata as $key => $val) {
            ...
            elseif ($no_processing) {
                $set[$no_processing_fields[$key]['table']]['`' . $no_processing_fields[$key]['field'] . '`'] = /* $db->escape($val, false,
                          $this->field_type[$key], $this->field_null[$key], isset($this->bit_field[$key])) */((isset($this->no_quotes[$key]) && isset($this->pass_var['create'][$key])) ? $db->escape($val, true) : $db->escape($val, false, $this->field_type[$key], $this->
                                        field_null[$key], isset($this->bit_field[$key])));
            }
        }
    ...
    protected function _update($postdata, $primary) {
        ...
        foreach ($postdata as $key => $val) {
            if (isset($fields[$key]) && !isset($this->locked_fields[$key])) {
                ...
                if (is_array($val)) {
                    $set[] = '`' . $fields[$key]['table'] . '`.`' . $fields[$key]['field'] . '` = ' . $db->escape(implode(',', $val), false, $this->field_type[$key], $this->field_null[$key], isset($this->bit_field[$key]));
                } elseif (isset($this->point_field[$key]) && trim($val)) {
                    $set[] = '`' . $fields[$key]['table'] . '`.`' . $fields[$key]['field'] . '` = Point(' . $db->escape($val, true, 'point', $this->field_null[$key], isset($this->bit_field[$key])) . ')';
                } else
                    $set[] = '`' . $fields[$key]['table'] . '`.`' . $fields[$key]['field'] . '` = ' . ((isset($this->no_quotes[$key]) && isset($this->pass_var['edit'][$key])) ? $db->escape($val, true) : $db->escape(trim($val), false, $this->field_type[$key], $this->field_null[$key], isset($this->bit_field[$key])));
            }
        }
        if (!$set) {
            ...
            if ($this->alert_create) {
                foreach ($this->alert_create as $alert) {
                    if ($alert['field'] && isset($postdata[$alert['field']]) && $postdata[$alert['field']] != $alert['value'])
                        continue;
                    if (!isset($postdata[$alert['column']]) or ! preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', $postdata[$alert['column']]))
                        continue;
        ...
        if ($this->inner_table_instance && ($mode == 'view' or $mode == 'edit')) { // restoring nested objects
            foreach ($this->inner_table_instance as $inst_name => $field) {
                if (isset($this->result_row[$field])) {
                    ...
                    $this->nested_rendered[$inst_name] = '<div class="xcrud-nested-container xcrud-container"><div class="xcrud-ajax" id="xcrud-ajax-' . base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }
    ...
    public function _export_vars() {
        ...
        if ($this->instance_count == 1 && !$this->ajax_request) {
            if (isset($_SESSION['xcrud_session']) && $_SESSION['xcrud_session']) {
                foreach ($_SESSION['xcrud_session'] as $s_key => $s_val) { // workaround on some servers session duplication
    ...
    protected function create_bool($name, $value = '', $tag = array()) {
        ...
        return $this->open_tag('div', $this->theme_config('checkbox_container')) . $this->open_tag('label') . $this->single_tag($tag, $this->theme_config('bool_field'), $this->field_attr[$name], true) . $this->close_tag('label') . $this->close_tag('div');
    }
    ...
    protected function _draw_watermark($src_file, $dest_file, $dest_qual = 95, $watermark = false, $watermark_position = array(50, 50)) {
        list($srcWidth, $srcHeight, $type) = getimagesize($src_file);
        switch ($type) {
            case 1:
                $srcHandle = imagecreatefromgif($src_file);
    ...
    protected function _compare($val1, $operator, $val2) {
        switch ($operator) {
            ...
            case '$=':
                return (mb_strpos($val1, $val2, 0, Xcrud_config::$mbencoding) == (mb_strlen($val1, Xcrud_config::$mbencoding) - mb_strlen($val2, Xcrud_config::$mbencoding))) ? true : false;
            case '~=':
        ...
        if (isset($this->field_type[$field])) {
            switch ($this->field_type[$field]) {
                ...
                case 'price':
                    ...
                    $out .= number_format($value ? $value : 0, (int) $this->field_attr[$field]['decimals'], $this->field_attr[$field]['point'], $this->field_attr[$field]['separator']);
                ...
                case 'image':
                    if ($value) {
                        if (Xcrud_config::$images_in_grid) {
                            ...
                            $out .= $this->open_tag('a', '', array('target' => '_blank', 'href' => $this->file_link($field, $primary_val, false, false, $value)));
    ...
    protected function _get_theme_config() { // loads theme configuration from ini file
    ...
    protected function lang($text = '') {
        ...
        return htmlspecialchars((isset($this->lang_arr[$langtext]) ? $this->lang_arr[$langtext] : $text), ENT_QUOTES, Xcrud_config::$mbencoding);
    }
        ...
        if (isset($this->field_tabs[$mode]) or $this->default_tab !== false) {
            ...
            if ($this->default_tab !== false && $raw_out) {
                ...
                $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->
                                                theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid)) . $this->default_tab . $this->
            ...
            if (isset($this->field_tabs[$mode]) && $tabs_out) {
                foreach ($this->field_tabs[$mode] as $key => $tabname) {
                    if (isset($tabs_out[$tabname])) {
                        $tabid = 'tabid_' . base_convert(rand(), 10, 36);
                        $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->
                                                        theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid)) . $tabname . $this->close_tag($tabs_link) .
