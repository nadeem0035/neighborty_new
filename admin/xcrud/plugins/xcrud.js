...
var Xcrud = {
	...
	current_task: null,
    publish_request: function(container, data, success_callback) {
		//console.log(data);
        ...
        jQuery.ajax({
            type: "post",
            url: Xcrud.config('url').replace("admin", "listings/publish_listing_notice"),
            beforeSend: function() {
	...
	request: function(container, data, success_callback) {
		...
		jQuery.ajax({
			...
			complete: function() {
				Xcrud.hide_progress(container);
				//console.log(data);
				Xcrud.publish_request(container,'id='+data.primary+'&action='+data.action)
			},
			dataType: "html",
			cache: false
		});
