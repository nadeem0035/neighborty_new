...
class CurlStripe {
	...
	public function __construct() {
		// Store the config values
                ...
                $this->_ci->load->config('stripe');
                $params['stripe_key_test_public'] = $this->_ci->config->item('stripe_key_test_public');
                $params['stripe_key_test_secret'] = $this->_ci->config->item('stripe_key_test_secret');
                $params['stripe_key_live_public'] = $this->_ci->config->item('stripe_key_live_public');
                $params['stripe_key_live_secret'] = $this->_ci->config->item('stripe_key_live_secret');
                $params['stripe_test_mode'] = $this->_ci->config->item('stripe_test_mode');
                $params['stripe_verify_ssl'] = $this->_ci->config->item('stripe_verify_ssl');
                $params['stripe_currency'] = $this->_ci->config->item('stripe_currency');
                $params['stripe_decode'] = $this->_ci->config->item('stripe_decode');
		$this->_conf = $params;
	}
...
// END CurlStripe Class

/* End of file CurlStripe.php */
/* Location: ./application/libraries/CurlStripe.php */